...
#SBATCH --job-name=mpi-28gpu-detr-large
##------------------------ End job description ------------------------
...
# Read the last checkpoint path from the file
# checkpoint_path=$(<last_checkpoint)
# ==> No need to read checkpoint path as it will automatically take the last one

mpirun \
    --allow-run-as-root \
    --np $WORLD_SIZE \
    --npernode $SLURM_GPUS_ON_NODE \
    python tools/train.py work_configs/RefSAM_det_coco_base_large.py \
            --launcher mpi \
            --resume
