...
#SBATCH --job-name=mpi-16gpu-detr-test
##------------------------ End job description ------------------------
...
export experim_name="RefSAM_det_coco_base"

mpirun \
    --allow-run-as-root \
    --np $WORLD_SIZE \
    --npernode $SLURM_GPUS_ON_NODE \
    python tools/test.py work_configs/$experim_name.py \
            work_dirs/$experim_name/epoch_50.pth \
            --launcher mpi
#            --show
